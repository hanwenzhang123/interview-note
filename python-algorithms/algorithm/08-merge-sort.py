Both Quicksort and Merge Sort are recursive. 
The difference between them is in the sorting mechanism itself. 
Whereas Quicksort sorts a list into two sub-lists that are less than or greater than a pivot value, 
Merge Sort simply splits the list in half recursively, and then sorts the halves as it merges them back together. 

Selection sort vs Quicksort vs Merge sort
selection sort takes many times longer, too slow for large data
quicksort and merge sort are similar time but quicksort better and faster
quicksort is the better solution for large data
...
What we're seeing are the values being returned from the recursive merge_sort function calls, not the original calls to merge_sort. So what you see here is after we reach the base case with a list that's only one item in length, and the recursive calls start returning.
The original list gets split into two unsorted halves: [4, 6, 2, 3], and [9, 7, 3, 5].
The first half gets split in half again: [4, 6] and [2, 3], and each of those halves is halved again into single-element lists: [4], [6], [2], and [3].
There's nothing to sort in the single-element lists, so they're returned from the merge_sort function as-is.
Those single-element lists get merged into two sub-lists: the [4, 6] and [2, 3] sub-lists look the same after sorting as they did before sorting.
But the order is shifted when those two sub-lists get merged back into a single list: [2, 3, 4, 6].
Then we recursively sort the right half of the list: [9, 7, 3, 5].
It gets split in half again: [9, 7] and [3, 5].
And each of those halves gets broken into single-element lists.
There's nothing to sort there, so they're returned as-is: [9], [7], [3], and [5]
The first two are sorted as they're merged: [7, 9].
The order of the second two is unchanged: [3, 5].
All those elements are sorted as they're merged into the second half of the full list: [3, 5, 7, 9].
And then the two halves are sorted as they're merged back into the full list: [2, 3, 3, 4, 5, 6, 7, 9]
