binary search works for sorted items
Binary search keeps narrowing down the list until it has the value it's looking for.0:13
It's faster than linear search because it discards half the potential matches each time.
...
It's important to give the script the name of the sorted file when you run it. If it loads the unsorted file, the binary search won't work. (See the "Sorting Names" video if you don't have a sorted.txt file saved.)

python binary_search.py names/sorted.txt
